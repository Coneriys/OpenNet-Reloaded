...
function route(recieverip, senderip, ...)
    ...
    if cl then
        ...
        lan[clientscard[cl]:sub(1, 3)]:sendrec(clients[cl], recieverip, senderip, ...)
    else
        -- Если не найден LAN клиент, используем WAN
        if wan then
            wan:sendrec(wan.router, recieverip, senderip, ...)
        else
...
function commands.getip()
    if lan[acceptedAdr:sub(1, 3)] then
        ...
        if not clients[adr] then
            ...
            lan[acceptedAdr:sub(1, 3)]:sendrec(senderAdr, adr, ip, "setip")
